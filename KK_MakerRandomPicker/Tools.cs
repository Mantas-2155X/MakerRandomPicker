...
namespace KK_MakerRandomPicker
{
    public static class Tools
    {
        ...
        public static void MakerAPI_RegisterCustomSubCategories(object sender, RegisterSubCategoriesEvent e)
        {
            ...
            e.AddControl(new MakerButton("Set current character as template", cat, KK_MakerRandomPicker.instance)).OnClick.AddListener(() =>
            {
                Randomizer.AssignTemplate(MakerAPI.GetCharacterControl());
            });

            var randButton = e.AddControl(new MakerButton("Randomize!", cat, KK_MakerRandomPicker.instance));

            e.AddControl(new MakerSeparator(cat, KK_MakerRandomPicker.instance));
            var randomizeBodySliders = e.AddControl(new MakerToggle(cat, "Randomize body sliders", KK_MakerRandomPicker.instance));
            var randomizeBody = e.AddControl(new MakerToggle(cat, "Randomize body type", KK_MakerRandomPicker.instance));
            var skinColorType = e.AddControl(new MakerRadioButtons(cat, KK_MakerRandomPicker.instance, "Skin color", "White", "Brown", "Unchanged"));

            e.AddControl(new MakerSeparator(cat, KK_MakerRandomPicker.instance));
            var randomizeFaceSliders = e.AddControl(new MakerToggle(cat, "Randomize face sliders", KK_MakerRandomPicker.instance));
            var randomizeFace = e.AddControl(new MakerToggle(cat, "Randomize face type", KK_MakerRandomPicker.instance));
            var randomizeFaceEyes = e.AddControl(new MakerToggle(cat, "Randomize eyes", KK_MakerRandomPicker.instance));

            e.AddControl(new MakerSeparator(cat, KK_MakerRandomPicker.instance));
            var randomizeHair = e.AddControl(new MakerToggle(cat, "Randomize hair type", KK_MakerRandomPicker.instance));
            var randomizeHairColor = e.AddControl(new MakerToggle(cat, "Randomize hair color", KK_MakerRandomPicker.instance));

            e.AddControl(new MakerSeparator(cat, KK_MakerRandomPicker.instance));
            var deviationSlider = e.AddControl(new MakerSlider(cat, "Deviation", 0, 1, 0.1f, KK_MakerRandomPicker.instance));
